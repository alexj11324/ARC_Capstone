...
2. **Rasterization**: `slosh_to_raster.py` converts raw SLOSH advisories into standardized GeoTIFF formats.
3. **Intersection**: `duckdb_fast_pipeline.py` / `fast_e2e_from_oracle.py` leverages Geopandas and DuckDB to merge SLOSH flooding grids with NSI structure locations.
